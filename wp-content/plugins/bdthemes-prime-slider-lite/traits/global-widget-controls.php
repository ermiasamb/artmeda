...
trait Global_Widget_Controls
{
    ...
    protected function register_social_links_controls_style()
    {
        ...
        $this->add_responsive_control( 'social_icon_horizontal_offset', [
    ...
    protected function register_swiper_effects_controls()
    {
        $this->add_control( 'swiper_effect', [
            'label'   => esc_html__( 'Swiper Effect', 'bdthemes-prime-slider' ) . BDTPS_NC,
            'type'    => Controls_Manager::SELECT,
            'default' => 'slide',
            'options' => [
        ...
        $this->add_control( 'creative_effect', [
    ...
    /**
     * Show Post Except controls
     */
    protected function register_show_post_except_controls()
    {
        $this->add_control( 'show_excerpt', [
    ...
    protected function register_loop_rewind_mousewheel_controls()
    {
        ...
        $this->add_control( 'rewind', [
            'label'     => __( 'Rewind', 'bdthemes-prime-slider' ) . BDTPS_NC,
            'type'      => Controls_Manager::SWITCHER,
            'default'   => 'yes',
            'condition' => [
            'swiper_effect' => [ 'slicer', 'tinder' ],
    ...
    public function render_sub_title( $slide, $class, $data_reveal )
    {
        ...
        echo  esc_attr( $class ) ;
        ?>" data-reveal="<?php 
        echo  esc_attr( $data_reveal ) ;
        ?>">
            <?php 
        echo  wp_kses( $slide['sub_title'], prime_slider_allow_tags( 'title' ) ) ;
        ?>
        </<?php 
        echo  Utils::get_valid_html_tag( $settings['sub_title_html_tag'] ) ;
        ?>>
     ...
     * Background image
     */
    public function rendar_item_image( $item, $alt = '', $class )
    {
        $settings = $this->get_settings_for_display();
    ...
    public function rendar_image( $slide, $reveal )
    {
		...
		<div class="bdt-image-wrap" <?php 
        echo  esc_attr( $reveal ) ;
        ?>>
		<?php 
        ...
        if ( !$thumb_url ) {
            printf(
                '<img src="%1$s" alt="%2$s" class="bdt-img %3$s">',
                $slide['image']['url'],
