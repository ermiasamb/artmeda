...
class Taxonomies
{
    ...
    private function getRegisteredTaxonomies()
    {
        $registeredTaxonomies = get_posts([
            'posts_per_page' => -1,
            'post_type' => Utils::getInfo('ui_prefix') . '_tax',
            'post_status' => 'publish'
        ]);
        ...
        return (array)apply_filters(Utils::getHookName('register_taxonomies'), $taxonomiesByUi);
    }
    ...
    public function initRegisteredTaxonomies()
    {
        ...
        foreach ($taxonomies as $i => $taxonomy) {
            ...
            $errorInfo = Utils::getRegistrationErrorNoticeInfo($taxonomy, 'tax');
            ...
            if(in_array($id, Utils::getTaxonomiesBlacklist())){
                add_filter(Utils::getHookName('register_notices'), function ($args) use ($errorInfo) {
                    $args[] = [
                        'id' => $errorInfo['id'],
                        'title' => Utils::getNoticesTitle(),
                        'message' => __('Taxonomy reserved or already registered, try a different "id".', 'custom-post-types') . $errorInfo['details'],
                        'type' => 'error',
                        'dismissible' => 3,
                        'admin_only' => 'true',
                        'buttons' => false,
            ...
            $registrationLabels = apply_filters(Utils::getHookName('register_tax_labels_' . $id), $registrationLabels);
            ...
            $registrationArgs = apply_filters(Utils::getHookName('register_tax_args_' . $id), $registrationArgs);
        ...
        Utils::flushRewriteRules($taxonomies);
    }
}