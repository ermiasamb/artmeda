...
class AdminNotices
{
    ...
    {
        // Notice ajax dismiss
        add_filter(Utils::getHookName('register_ajax_actions'), function ($actions) {
    ...
    {
        $dismissed_notices = get_option(Utils::getOptionName('dismissed_notices'), []);
    ...
    private function dismissNotice($id, $days = 2)
    {
        ...
        update_option(Utils::getOptionName('dismissed_notices'), $dismissed_notices);
    }
    ...
    private function getRegisteredNotices(){
        $registeredNotices = get_posts([
            'posts_per_page' => -1,
            'post_type' => Utils::getInfo('ui_prefix') . '_notice',
            'post_status' => 'publish'
        ]);
        ...
        foreach ($registeredNotices as $notice) {
            ...
            $noticeAdminOnly = get_post_meta($notice->ID, 'admin_only', true) == 'true';
        ...
        return (array)apply_filters(Utils::getHookName('register_notices'), $noticesByUi);
    }
    ...
    public function initRegisteredNotices(){
        ...
        foreach($notices as $i => $notice){
            ...
            $adminOnly = !empty($notice['admin_only']) ? $notice['admin_only'] : false;
            if (
