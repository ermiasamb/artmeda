...
class PostTypes
{
    ...
    private function getRegisteredPostTypes()
    {
        $registeredPostTypes = get_posts([
            'posts_per_page' => -1,
            'post_type' => Utils::getInfo('ui_prefix'),
            'post_status' => 'publish'
        ]);
        ...
        $postTypesByCore = Utils::getUiPostTypesArgs();

        $postTypesByPlugin = (array)apply_filters(Utils::getHookName('register_post_types'), $postTypesByUi);

        return array_merge($postTypesByCore, $postTypesByPlugin);
    }
    ...
    public function initRegisteredPostTypes()
    {
        ...
        foreach ($postTypes as $i => $postType) {
            ...
            $errorInfo = Utils::getRegistrationErrorNoticeInfo($postType);
            ...
            if(in_array($id, Utils::getPostTypeBlacklist())){
                add_filter(Utils::getHookName('register_notices'), function ($args) use ($errorInfo) {
                    $args[] = [
                        'id' => $errorInfo['id'],
                        'title' => Utils::getNoticesTitle(),
                        'message' => __('Post type reserved or already registered, try a different "id".', 'custom-post-types') . $errorInfo['details'],
                        'type' => 'error',
                        'dismissible' => 3,
                        'admin_only' => 'true',
                        'buttons' => false,
            ...
            $registrationLabels = apply_filters(Utils::getHookName('register_labels_' . $id), $registrationLabels);
            ...
            $registrationArgs = apply_filters(Utils::getHookName('register_args_' . $id), $registrationArgs);
        ...
        Utils::flushRewriteRules($postTypes);
    }
}