...
if ( !class_exists( 'Better_Messages_Hooks' ) ):
    ...
    class Better_Messages_Hooks
    {
        ...
        public function __construct()
        {
            ...
            if( isset(Better_Messages()->settings['restrictRoleBlock'])
            ...
            */
            ...
            #if( Better_Messages()->settings['enablePinnedMessage'] === '1' ) {
        ...
        public function role_to_role_block_search_user_sql_condition( $sql_array, $user_ids, $search, $user_id ){
            ...
            if( count( $restrict_to ) > 0 ){
                ...
                foreach( $roles as $role ){
                    $holders[] = '`meta_value` LIKE %s';
                    $arguments[] = '%'. $role .'%';
                }

                $sql = "AND `ID` NOT IN(SELECT user_id FROM `{$wpdb->usermeta}` WHERE `meta_key` = 'wp_capabilities' AND (" . implode(' OR ', $holders) . "))";

                $sql_array[] = $wpdb->prepare($sql, $arguments);
            }
        ...
        public function override_last_activity($null, $object_id, $meta_key, $meta_value, $prev_value){
            if( $meta_key === 'last_activity' ) {
                Better_Messages()->functions->update_user_meta($object_id, 'bpbm_last_activity', $meta_value);
            }
