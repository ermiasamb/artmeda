...
if ( !class_exists( 'Better_Messages_Guests' ) ):
    ...
    class Better_Messages_Guests
    {
        ...
        public function delete_guest_user( int $user_id ){
            ...
            if( $guest ){
                $wpdb->query( $wpdb->prepare( "UPDATE `" . bm_get_table('guests') . "` SET `last_changed` = %d, `deleted_at` = current_timestamp() WHERE `id` = %d", Better_Messages()->functions->get_microtime(), $guest->id ) );
