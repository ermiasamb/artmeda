    ...
    class Better_Messages_Rest_Api
    {
        ...
        public function checkNew( WP_REST_Request $request ) {
            ...
            if( $lastClient > 0 ) {
                ...
                $changedGuests = $this->get_changed_guests( $lastClient );

                if( count( $changedGuests ) > 0 ) {
                    foreach ( $changedGuests as $guest_id ){
                        $return['users'][] = Better_Messages()->functions->rest_user_item( $guest_id );
                    }
                }
        ...
        public function send_message( WP_REST_Request $request ){
            ...
            if( ! empty( $temp_id ) ) {
                ...
                if ( ! empty( $temp_time ) ) {
                    $time_diff = time() - strtotime($temp_time);

                    if( $time_diff < 60 && $time_diff > -60 ) {
                        $args['date_sent'] = $temp_time;
                    }
                }
            }
        ...
        public function get_suggestions( WP_REST_Request $request ){
            ...
            $suggestions = Better_Messages_Search()->get_users_results( $search, $current_user_id );
        ...
        public function get_messages( $thread_id = null, $message_ids = [], $added_users = [], $count = 50, $apply_filters = true ){
            global $wpdb;
        ...
        public function get_changed_guests( $since ){
            global $wpdb;

            $guest_ids = array_map( 'intval', $wpdb->get_col( $wpdb->prepare( "SELECT id FROM `" . bm_get_table('guests') . "` WHERE `last_changed` >= %d", $since ) ) );
        ...
        public function load_more( WP_REST_Request $request ){
            ...
            if( $mode === 'until' ){
                $first = (int) $wpdb->get_var( $wpdb->prepare("SELECT id
                FROM `" . bm_get_table('messages') . "` 
                WHERE `thread_id` = %d 
                ORDER BY `id` DESC
                LIMIT %d, 1", $thread_id, $to - 1 ) );

                $from = range( $first, $from );
            } else if( $from > 0 && $to > 0 ){
                $from = range( $from, $to );
            }
        ...
        public function search_participants( WP_REST_Request $request ){
            ...
            OR `recipients`.`user_id` IN (
                SELECT -1 * ABS(id)
                FROM `" . bm_get_table('guests') . "` `guests`
                WHERE `guests`.`name` LIKE %s
            ) )", $thread_id, $search, $search, $search, $search);
        ...
        public function get_threads( $thread_ids = [], $fetch_messages = true, $fetch_users = true, $personal_data = true, $cache = true, $user_id = false ){
            ...
            foreach ( $get_threads as $thread ){
                ...
                $thread_item =  [
                    'thread_id'          => $thread_id,
                    'lastMessage'        => (int) $thread->message_id,
                ...
                if( $personal_data ){
                    ...
                    $thread_item['permissions'] = [
                        ...
                        'canAudioCall'         => false,
                        'canMaximize'          => true,
                        'canMinimize'          => is_user_logged_in() && Better_Messages()->settings['miniChatsEnable'] === '1',
