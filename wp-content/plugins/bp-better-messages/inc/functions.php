...
if ( !class_exists( 'Better_Messages_Functions' ) ):
    ...
    class Better_Messages_Functions
    {
        ...
        public function get_link( $user_id = false )
        {
            ...
            if( class_exists('BuddyPress') && function_exists('bp_core_get_user_domain') && $user_id !== $current_user_id ){
        ...
        public function get_friends_sorted( $user_id, $count = 'all' ){
            ...
            $query = "SELECT user_id, meta_value 
            FROM {$wpdb->usermeta} usermeta
            RIGHT JOIN {$wpdb->users} users
                ON `users`.`ID` = `usermeta`.`user_id`
            WHERE `user_id` IN (" . implode( ',', array_keys( $last_active_users ) ) . ") 
            AND `meta_key` = 'bpbm_last_activity' 
        ...
        public function get_users_sorted( $user_id, $exclude = [], $count = 10 ){
            ...
            RIGHT JOIN {$wpdb->users} users
                ON `users`.`ID` = `usermeta`.`user_id`
            WHERE `user_id` != %d 
                AND " . $excluded_sql . " `meta_key` = 'bpbm_last_activity' 
                ORDER BY `meta_value` DESC", $user_id);
            $query .= $wpdb->prepare(' LIMIT 0, %d', $count );
        ...
        public function new_message( $args = '' ) {
            ...
            $this->update_message_update_time( $message->id, true );
        ...
        public function update_message_update_time( $message_id, $new_message = false, $deleted_message = false ){
        ...
        public function get_last_activity( $user_id ){
            $last_activity = Better_Messages()->functions->get_user_meta( $user_id, 'bpbm_last_activity', true  );
