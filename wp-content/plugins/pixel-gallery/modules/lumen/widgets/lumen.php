...
namespace PixelGallery\Modules\Lumen\Widgets;
...
class Lumen extends Module_Base {
	...
	public function get_title() {
		return BDTPG . esc_html__('Lumen', 'pixel-gallery');
	}

	public function get_icon() {
		return 'pg-icon-lumen pg-new';
	}
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
            'item_perspective',
            [
                'label'   => __( 'Perspective', 'pixel-gallery' ),
                ...
                'selectors' => [
					'{{WRAPPER}} .pg-lumen-item' => 'perspective: {{SIZE}}{{UNIT}};',
				],
            ]
        );
		$this->register_title_tag_controls();
		$this->register_show_meta_controls();
		$this->register_content_alignment_controls('lumen');
		...
		$this->register_repeater_grid_controls($repeater, 'lumen');
		$this->register_repeater_item_height_controls($repeater, 'lumen');
		$repeater->end_controls_tab();
		...
		$this->start_controls_section(
			'pg_section_style',
			[
				'label'     => esc_html__( 'Item', 'pixel-gallery' ),
		...
		$this->add_group_control(
			...
			[
				...
				'label' => esc_html__( 'Image Overlay', 'pixel-gallery' ),
				'types' => [ 'classic', 'gradient' ],
				'exclude' => [ 'image' ],
				'selector' => '{{WRAPPER}} .pg-lumen-item',
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .pg-lumen-item' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .pg-lumen-item' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .pg-lumen-item:hover' => 'border-color: {{VALUE}};',
				],
			]
		);
        ...
        $this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .pg-lumen-item:hover',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors'   => [
					'{{WRAPPER}} .pg-lumen-content' => 'backdrop-filter: blur({{SIZE}}px); -webkit-backdrop-filter: blur({{SIZE}}px);'
				],
				'condition' => [
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .pg-lumen-content ',
				'separator' => 'before',
				'fields_options' => [
					'background' => [
						'default' => 'classic',
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .pg-lumen-content' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .pg-lumen-content' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .pg-lumen-content' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->register_title_controls('lumen');
	...
	public function render_items() {
		...
		$id = 'pg-lumen-' . $this->get_id();
		$slide_index = 1;
		foreach ($settings['items'] as $index => $item) :

			$attr_name = 'grid-item' . $index;
			$this->add_render_attribute($attr_name, 'class', 'pg-lumen-item pg-item elementor-repeater-item-' . esc_attr($item['_id']), true);
		...
		<div <?php $this->print_render_attribute_string($attr_name); ?>>
			...
			<?php $this->render_image_wrap($item, 'lumen'); ?>
			<div class="pg-lumen-content">
				<?php $this->render_title($item, 'lumen'); ?>
				<?php $this->render_meta($item, 'lumen'); ?>
			</div>
