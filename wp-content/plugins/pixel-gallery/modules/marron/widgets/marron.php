...
namespace PixelGallery\Modules\Marron\Widgets;
...
class Marron extends Module_Base
{
	...
	{
		return BDTPG . esc_html__('Marron', 'pixel-gallery');
	}

	public function get_icon()
	{
		return 'pg-icon-marron';
	}
	...
	protected function register_controls()
	{
		...
		//Global
		$this->register_grid_controls('marron');
		$this->register_global_height_controls('marron');
		$this->register_title_tag_controls();
		$this->register_show_meta_controls();
		$this->register_show_date_controls();
		$this->register_alignment_controls('marron');
		...
		$this->register_repeater_grid_controls($repeater, 'marron');
		$this->register_repeater_item_height_controls($repeater, 'marron');
		$repeater->end_controls_tab();
		...
		$this->add_control(
			...
			[
				...
				'selectors'   => [
					'{{WRAPPER}} .pg-marron-image-wrap::before' => 'backdrop-filter: blur({{SIZE}}px); -webkit-backdrop-filter: blur({{SIZE}}px);'
				],
				'condition' => [
		...
		$this->add_group_control(
			...
			[
				'name'      => 'item_border',
				'selector'  => '{{WRAPPER}} .pg-marron-image-wrap',
				'separator' => 'before',
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors'  => [
					'{{WRAPPER}} .pg-marron-image-wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors'  => [
					'{{WRAPPER}} .pg-marron-image-wrap' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors'  => [
					'{{WRAPPER}} .pg-marron-image-wrap' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors'  => [
					'{{WRAPPER}} .pg-marron-content' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors'  => [
					'{{WRAPPER}} .pg-marron-content' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors'  => [
					'{{WRAPPER}} .pg-marron-content' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->register_title_controls('marron');

		//Global meta Controls
		$this->register_meta_controls('marron');

		//Global date Controls
		$this->register_date_controls('marron');
	...
	public function render_items()
	{
		...
		$id = 'pg-marron-' . $this->get_id();
		$slide_index = 1;
		foreach ($settings['items'] as $index => $item) :

			$attr_name = 'grid-item' . $index;
			$this->add_render_attribute($attr_name, 'class', 'pg-marron-item pg-item elementor-repeater-item-' . esc_attr($item['_id']), true);
			...
			<div <?php $this->print_render_attribute_string($attr_name); ?>>
				...
				<div class="pg-marron-image-wrap">
					<?php
						 ...
						 */
						if ($item['media_type'] == 'video') {
							$this->render_poster($item, 'marron');
						} else {
							$this->render_image($item, 'marron');
						}
					?>
						<?php $this->render_date($item, 'marron'); ?>
					</div>
					<div class="pg-marron-content">
						<?php $this->render_title($item, 'marron'); ?>
						<div class="pg-marron-meta-wrap">
							<?php $this->render_meta($item, 'marron'); ?>
							<?php if ('none' !== $settings['link_to'] && $settings['link_target'] == 'only_button') : ?>
								<?php $this->render_readmore($item, $index, $id, 'marron'); ?>
							<?php endif; ?>
						</div>
					</div>
	...
	{
		$settings   = $this->get_settings_for_display();
		$this->add_render_attribute('grid', 'class', 'pg-marron-grid pg-grid');
